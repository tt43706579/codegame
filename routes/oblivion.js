...
// --------------------------------------------------失落帝國（地牢）--------------------------------------------------------------------------
router.get('/oblivion_dungeon', ensureAuthenticated, function(req, res, next) {
  res.render('oblivion_dungeon/oblivion_dungeon', {
    user: req.user.name
  });
});


router.post('/oblivion_dungeon', function(req, res, next) {
  var type = req.body.type
  if (type == "init") {
    var id = req.user.id;
    User.getUserById(id, function(err, user) {
