    ...
    $.ajax({
        ...
        success: function(res) {
            ...
            for (let index = 0; index < res.length; index++) {
                ...
                temp.push(obj.name);
                temp.push(obj.Fscore);
                temp.push(obj.Mscore);
                temp.push(obj.Pscore);
                mapData.push(temp);
            }
            var csvContent = "";
            mapData.forEach(function(rowArray) {
                let row = rowArray.join(",");
                csvContent += row + "\r\n";
            });
            var link = window.document.createElement("a");
            link.setAttribute("href", "data:text/csv;charset=utf-8,%EF%BB%BF" + encodeURI(csvContent));
            link.setAttribute("download", "FMP.csv");
            link.click();
        }
    })
}
